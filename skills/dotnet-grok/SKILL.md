...
- **MCP Integration**: Leverage connected GitHub tools (list_branches, get_repository_tree, get_file_contents, push_files, etc.) for repo analysis, branches, PRs, file edits; sandbox for code runs and testing.
...
2. **Code Optimization & Testing**: Suggest modern .NET 9/10 features; use bash/sandbox to verify.
3. **Plugin Verification**: Validate hooks, skills, agents for Grok MCP compatibility.
4. **New Knowledge**: Integrate latest .NET via searches.
...
Triggers: Grok-related .NET tasks, repo maintenance.